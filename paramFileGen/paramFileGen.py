...
class MainWindow(QtGui.QWidget):
            ...
            for param in val1:
                ...
                for key2, val2 in param.iteritems():
                    ...
                    if len(val2) == 1:
                        ...
                        self.secondaryLayout.addWidget(self.paramName, index,
                                                       0)
                        ...
                        for index2 in range(0, len(val2)):
                            if index2 > 0:
                                ...
                                self.secondaryLayout.addWidget(
                                    self.comboBox, index, 1)
    ...
    def loadFile(self):
        ...
        loadFileName = QtGui.QFileDialog.getOpenFileName(
            self, 'Load file',
            '/Users/soorinpark/Documents/School/ShaheenGroup/OPV_GUI')
    ...
    def saveFile(self):
        ...
        for index, value in enumerate(self.paramDic):
            ...
            # Electron checking
            # X
            if (self.paramDic[index][0].text() == "PeriodicX" and
                    self.paramDic[index][1].currentText() == "1"):
                #print "1", self.paramDic[index][1].currentText()
                period = 1
            if (self.paramDic[index][0].text() == "XElecOn" and
                    self.paramDic[index][1].currentText() == "1"):
                #print "2", self.paramDic[index][1].currentText()
                elec = 1
                xon = 1
            if (self.paramDic[index][0].text() == "EndX" and
                    self.paramDic[index][1].currentText() == "0"):
                #print "3", self.paramDic[index][1].currentText()
                end = 1
                xyz = "X"

            # Y
            if (self.paramDic[index][0].text() == "PeriodicY" and
                    self.paramDic[index][1].currentText() == "1"):
                #print "1", self.paramDic[index][1].currentText()
                period = 1
            if (self.paramDic[index][0].text() == "YElecOn" and
                    self.paramDic[index][1].currentText() == "1"):
                #print "2", self.paramDic[index][1].currentText()
                elec = 1
                yon = 1
            if (self.paramDic[index][0].text() == "EndY" and
                    self.paramDic[index][1].currentText() == "0"):
                #print "3", self.paramDic[index][1].currentText()
                end = 1
                xyz = "Y"
            ...
            # Z
            if (self.paramDic[index][0].text() == "PeriodicZ" and
                    self.paramDic[index][1].currentText() == "1"):
                #print "1", self.paramDic[index][1].currentText()
                period = 1
            if (self.paramDic[index][0].text() == "ZElecOn" and
                    self.paramDic[index][1].currentText() == "1"):
                #print "2", self.paramDic[index][1].currentText()
                elec = 1
                zon = 1
            if (self.paramDic[index][0].text() == "EndZ" and
                    self.paramDic[index][1].currentText() == "0"):
                #print "3", self.paramDic[index][1].currentText()
                end = 1
                xyz = "Z"
        ...
        if dataisnotthere:
            QtGui.QMessageBox.about(
                self, "Warning",
                "Some parameters have not been entered. Please go back and check that all parameters have been filled in."
            )
            return
            ...
            QtGui.QMessageBox.about(
                self, "Warning",
                "Periodic%s = 1, %sElecOn = 1, therefore End%s cannot be 0. Please go back and edit End%s."
                % (xyz, xyz, xyz, xyz))
            return
            ...
            QtGui.QMessageBox.about(
                self, "Warning",
                "At least one set of electrodes (*ElecOn) must be on")
            return
        ...
        with open(saveFileName, 'a') as saveFile:
            for index, value in enumerate(self.paramDic):
                try:
                    ...
                    data = self.paramDic[index][0].text(
                    ) + " " + self.paramDic[index][1].text() + "\n"
                    print(data)
                    saveFile.write(data)
                    ...
                    data = self.paramDic[index][0].text(
                    ) + " " + self.paramDic[index][1].currentText() + "\n"
                    print(data)
                    saveFile.write(data)
